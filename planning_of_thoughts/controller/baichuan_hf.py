...
class Baichuan(AbstractLanguageModel):
    """
    An interface to use LLaMA 2 models through the HuggingFace library.
    """

    def __init__(
        self, config_path: str = "", model_name: str = "Baichuan2-7B", cache: bool = False
    ) -> None:
        """
        ...
        if "loacl_model_path" in self.config:
            hf_model_id = os.path.join(self.config['loacl_model_path'],self.model_id)
        else:
            hf_model_id = f"meta-llama/{self.model_id}"
        model_config = transformers.AutoConfig.from_pretrained(hf_model_id,trust_remote_code=True)
        ...
        print(hf_model_id)
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(hf_model_id,trust_remote_code=True)
        ...
        self.model.generation_config = GenerationConfig.from_pretrained(hf_model_id,trust_remote_code=True)
        self.model.eval()
        torch.no_grad()

        self.generate_text = self.model.chat
    ...
    def query(self, query: str, num_responses: int = 1) -> List[Dict]:
        ...
        for _ in range(num_responses):
            sequences.extend(
                [self.generate_text(
                    self.tokenizer,
                    [
                        # {"role": "system", "content": "解释一下“温故而知新”"},
        ...
        response = [
            {"generated_text": ''.join(sequence).strip()}

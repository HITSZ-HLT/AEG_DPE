...
from planning_of_thoughts.operations import GraphOfOperations, Thought
...
class Controller:
    ...
    def run(self) -> None:
        ...
        while len(execution_queue) > 0:
            ...
            # current_operation.executed=True
            ...
            for operation in current_operation.successors:
                ...
                ), "The successor of an operation is not in the operations graph{}".format(operation)
                if operation.can_be_executed():
                    execution_queue.append(operation)
                else:
                    logging.warning(f"{operation} is a successor of {current_operation} but can't be enqueued because of predecessors unexecuted")

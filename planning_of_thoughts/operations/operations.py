...
from planning_of_thoughts.operations.thought import Thought
from planning_of_thoughts.controller.abstract_language_model import AbstractLanguageModel
from planning_of_thoughts.prompter import Prompter
from planning_of_thoughts.parser import Parser
...
class Operation(ABC):
    ...
    def add_predecessor(self, operation: Operation | List[Operation]) -> None:
        """
        ...
        if isinstance(operation,List):
            for op in operation:
                self.predecessors.append(op)
                op.successors.append(self)
        else:
            self.predecessors.append(operation)
            operation.successors.append(self)

    def add_successor(self, operation: Operation| List[Operation]) -> None:
        """
...
class RefineMajorClaim(Operation):
    """
    Imporve MajorClaim from all claims and replace major claim
    """
...
class Refine(Operation):
    """
    Imporve Claim from Rebuttal and replace claim
    """
    ...
    def _execute(
        ...
        prompt = prompter.improve_prompt(previous_thought_states, target_type=self.target_type)
        ...
        parsed = parser.parse_improve_answer(previous_thought_states, responses, target_type=self.target_type)
...
class Integration(Operation):
    ...
    operation_type: OperationType = OperationType.others
    ...
    def _execute(
        ...
        previous_thought_states = [thought.state for thought in previous_thoughts]
        prompt = prompter.integ_prompt(previous_thought_states,target_type = self.target_type)
        ...
        parsed = parser.parse_integ_answer(previous_thought_states, responses)
...
class HeterGenerate(Operation):
    ...
    def __init__(
        self, num_branches_prompt: int = 1, num_branches_response: int = 1, target_type: str = "claim"
    ) -> None:
        """
    ...
    def _execute(
        ...
        for thought in previous_thoughts:
            base_state = thought.state
            prompt = prompter.generate_prompt(self.num_branches_prompt, **base_state,target_type=self.target_type)
            self.logger.debug("Prompt for LM: %s", prompt)
            responses = lm.get_response_texts(
                lm.query(prompt, num_responses=self.num_branches_response)
            )
            self.logger.debug("Responses from LM: %s", responses)
            for new_state in parser.parse_generate_answer(base_state, responses, target_type=self.target_type):
                new_state = {**base_state, **new_state}
                self.thoughts.append(Thought(new_state))
                self.logger.debug(
                    "New thought %d created with state %s",
...
class Generate(Operation):
    ...
    def __init__(
        self, num_branches_prompt: int = 1, num_branches_response: int = 1, target_type: str = None
    ) -> None:
        """
        Initializes a new Generate operation.
...
class HeterAggregate(Operation):
    """
    ...
    def __init__(self, num_responses: int = 1, target_type: str = 'counter_rebuttal') -> None:
        """
    ...
    def _execute(
        ...
        previous_thought_states = [thought.state for thought in previous_thoughts]
        prompt = prompter.aggregation_prompt(previous_thought_states, target_type=self.target_type)
        ...
        parsed = parser.parse_aggregation_answer(previous_thought_states, responses, target_type = self.target_type)
...
class Combine(Operation):
    ...
    def __init__(self, num_responses: int = 1, target_type: str = 'essay') -> None:
        """
        Initializes a new Aggregate operation.
    ...
    def _execute(
        ...
        # directly combine
        essay_components = {"introduction":[],"paragraph":[],"conclusion":[]}
        for state in previous_thought_states:
            assert state["type"] in essay_components, "essay failed"
            essay_components[state["type"]].append(state['current'])
        for k, v in essay_components.items():
            essay_components[k]="\n".join(v)
        essay = "{introduction}\n {paragraph}\n {conclusion}".format(**essay_components)
        essay_combine_dict = {"current":essay,'type':"essay", "part":"essay_combine"}
        if isinstance(parsed, dict):
            parsed = [parsed]
        for new_state in parsed:
            self.thoughts.append(Thought({**base_state, **new_state}))
        self.thoughts.append(Thought({**base_state, **essay_combine_dict}))
...
class RenameThoughts(Operation):
    """
    ...
    def rename(self) -> List[Thought]:
        ...
        # thought should be the same part
        
        parts = [t.state['part'].split(' ')[0] for t in previous_thoughts]
        unique_part = parts[0]
        for p in parts:
            assert p == unique_part
        
        for id_, thought in enumerate(previous_thoughts):
            thought.state['part'] = unique_part+' {}'.format(id_+1)
    ...
    def _execute(
        ...
        self.thoughts = [Thought.from_thought(thought) for thought in self.rename()]
...
class FilterN(Operation):
    ...
    def get_best_n(self) -> List[Thought]:
        ...
        #     previous_thought.scored for previous_thought in previous_thoughts
        # ), "Not all thoughts have been scored"
        if self.n < len(previous_thoughts):
            previous_thoughts = random.sample(previous_thoughts,k = self.n)
        return previous_thoughts
